    ...
    with tab2:
        ...
        if not st.session_state.model_loaded:
                ...
                def transform(self, frame):
                    ...
                    if detected_faces and (current_time - self.last_update_time) > self.detection_interval:
                        ...
                        st.session_state.detection_history.insert(0, {
                            ...
                            "probability": self.snapshot["probability"],
            ...
            if ctx.video_transformer:
                if ctx.video_transformer.snapshot:
                    snapshot = ctx.video_transformer.snapshot
                    st.subheader("💾 Snapshot Terakhir")
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.image(snapshot["frame"], caption=f"Snapshot - {snapshot['time']}", channels="BGR")
                    ...
                    with col2:
                        ...
                        # Tombol unduh
                        if st.button("📥 Unduh Snapshot"):
                            buf = cv2.imencode('.jpg', snapshot["frame"])[1].tobytes()
                            st.download_button(
                                label="Download",
                                data=buf,
                                file_name=f"snapshot_{snapshot['time']}.jpg",
                                mime="image/jpeg"
                            )
